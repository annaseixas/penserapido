...
def tela_quiz(root):
    ...
    for cat in categorias:
        btn_cat = ctk.CTkButton(frame, text=cat, font=("Arial", 22, "bold"), command=lambda c=cat: tela_quiz_perguntas(root, c), width=300, height=50, fg_color="orange")
        btn_cat.pack(pady=10)

    btn_voltar = ctk.CTkButton(frame, text="Voltar ao Menu", font=("Arial", 16, "bold"), command=lambda: voltar_menu(root), fg_color="orange")
    btn_voltar.pack(pady=20)

def mostrar_resposta(frame, resposta):
    label_resposta = ctk.CTkLabel(frame, text=f"Resposta: {resposta}", font=("Arial", 22, "bold"), text_color="green", wraplength=1000)
    label_resposta.pack(pady=20)

def proxima_pergunta(root, categoria):
    tela_quiz_perguntas(root, categoria)


def tela_quiz_perguntas(root, categoria):
    clear_screen(root)
    frame = ctk.CTkFrame(root)
    frame.pack(fill="both", expand=True)
    ...
    try:
        conn = conectar()
        cursor = conn.cursor()
        cursor.execute("SELECT id, pergunta, resposta FROM perguntas WHERE categoria = %s ORDER BY RAND() LIMIT 1", (categoria,))
        resultado = cursor.fetchone()

        if resultado:
            id_pergunta, pergunta, resposta_correta = resultado
            cursor.execute("SELECT resposta FROM perguntas WHERE categoria = %s AND id != %s ORDER BY RAND() LIMIT 3", (categoria, id_pergunta))
            wrong_answers = [row[0] for row in cursor.fetchall()]

            options = wrong_answers + [resposta_correta]
            ...
            btn_proxima= ctk.CTkButton(frame, text="Proxima Pergunta", font=("Arial", 22, "bold"), command=lambda: proxima_pergunta(root, categoria), width=200, height=50, fg_color="orange")
            btn_proxima.pack(pady=10)

            btn_resposta= ctk.CTkButton(frame, text="Mostrar Resposta", font=("Arial", 22, "bold"), command=lambda: mostrar_resposta(frame, resposta_correta), width=200, height=50, fg_color="orange")
            btn_resposta.pack(pady=10)

        else:
            erro = ctk.CTkLabel(frame, text="Não há perguntas cadastradas para esta categoria.", text_color="red", font=("Arial", 12))
            erro.pack(pady=20)

        conn.close()

    except mysql.connector.Error as err:
        erro = ctk.CTkLabel(frame, text=f"Erro: {str(err)}", text_color="red")
        erro.pack(pady=10)
